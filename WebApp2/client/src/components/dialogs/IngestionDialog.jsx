...
export default function IngestionDialog(props)
{
    ...
    function createPropCtrl(req)
    {
        ...
        if(req.type === 'Bool')
            return <FormControlLabel margin='dense' fullWidth control={<Checkbox inputProps={{onChange: handleCbChange}} id={req.name} checked={req.name in object && object[req.name].toString() === "true"} />} label={req.name} />
        if(req.type === 'Int')
